...
- [ ] **CF-101 — Toast in-app**: sistema di notifiche non-bloccanti (`saved`/`exported`/`validated`/errore) con `aria-live`, auto-dismiss + eventuale azione, al posto del solo testo nella status bar. Helper `toast()` riusabile, testato lato geometria/logica dove possibile.

...
- [ ] **CF-113 — Apri issue su GitHub/GitLab da un finding**: il follow-up deferito da CF-106. Da un finding BAD/ERROR crea una issue con titolo/corpo precompilati (check, target, messaggio, timestamp) via REST del forge; host/repo da config o env, **token solo da env** (`GITHUB_TOKEN`/`GITLAB_TOKEN`), mai dalla UI. Binding con TDD via `httptest` (payload, auth header, errore/duplicato). Zero-dep (HTTP/JSON a mano).

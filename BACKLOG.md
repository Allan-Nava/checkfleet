...
- [ ] **CF-106 — Azioni "Send to…" dalla GUI**: menu per inoltrare il run corrente a Slack / Discord / Teams / webhook riusando i renderer `internal/output`, e aprire/chiudere issue (riuso della logica `report-issues`). URL/token **solo da env** (mai in UI); dry-run e feedback via toast (CF-101). Binding `App.Send(target)` con `Plan` testabile.

...
- [ ] **CF-105 — Config editor v2**: estende il form "Add endpoint" a tutti i check comuni (oltre http/certs/tcp/dns: `redis`, `postgres`, `nats`, `tls`, `smtp`, `grpc`, `dns`…) via `engine.AddEndpoint`; **validazione live** mentre scrivi (debounce su `ValidateText`) con marcatori d'errore inline; picker dei moduli con snippet da `internal/scaffold`. Preserva commenti/formattazione dello YAML.

...
- [ ] **CF-91 — Metriche numeriche su `Finding`**: campo opzionale `Value float64` + `Unit string` su `engine.Finding` (JSON `omitempty`, backward-compatible: i renderer `internal/output` restano invariati e lo mostrano solo se presente). Popolato dai moduli che già misurano una grandezza — latenza (`http`, `tcp`, `tls`, `dns`, `grpc`, `ntp`, `redis`), giorni-a-scadenza (`certs`), lag (`patroni`, `postgres`, `mysql`, `clickhouse`) — senza cambiarne lo Status. `internal/history` persiste il valore per il charting nel tempo. Fondamenta per CF-94. Test engine + moduli toccati.

...
- [ ] **CF-95 — Availability / SLO**: `App.Availability` — % di run in stato OK su una finestra (per target e globale) e tempo trascorso nello stato corrente, derivati dallo storico (riusa `internal/history` + `engine.DiffStatus`); **tile SLO** nella Dashboard. Binding testato.

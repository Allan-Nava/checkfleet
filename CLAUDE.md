...
- Dipendenze: `gopkg.in/yaml.v3` (config). I moduli che parlano un protocollo di dominio senza equivalente stdlib usano un driver dedicato — eccezioni motivate: `jackc/pgx` (postgres/SQL), `go-ldap/ldap` (ldap/ASN.1-BER), `twmb/franz-go` (kafka), `go.mongodb.org/mongo-driver/v2` (mongodb: wire protocol BSON + auth SCRAM). Tutto il resto è **zero-dep** e resta tale: HTTP/JSON (nats, haproxy, consul, patroni, stream, keycloak, rabbitmq) e protocolli semplici scritti a mano (dns, redis/RESP, ntp/SNTP, grpc/health su HTTP2, tcp, tls) NON devono introdurre driver. Aggiungerne di nuovi solo con forte motivazione.

...
func (c *Check) Run(ctx context.Context) []engine.Finding {
	...
	if err != nil {
		return []engine.Finding{{Check: c.Name(), Target: "cluster", Status: engine.ERROR, Message: fmt.Sprintf("connessione fallita: %v", err)}}
	}
	defer cl.Close()
	...
	if err != nil {
		findings = append(findings, engine.Finding{Check: c.Name(), Target: "cluster", Status: engine.ERROR, Message: fmt.Sprintf("metadata fallito: %v", err)})
	} else {
...
func (c *Check) clusterFinding(m metadata) engine.Finding {
	...
	case !m.ControllerPresent:
		f.Status, f.Message = engine.BAD, fmt.Sprintf("nessun controller (%d broker)", m.Brokers)
	case c.cfg.ExpectBrokers > 0 && m.Brokers < c.cfg.ExpectBrokers:
		f.Status, f.Message = engine.WARN, fmt.Sprintf("%d/%d broker (mancanti)", m.Brokers, c.cfg.ExpectBrokers)
	default:
		f.Status, f.Message = engine.OK, fmt.Sprintf("%d broker, controller presente", m.Brokers)
	}
	return f
}
...
func (c *Check) partitionsFinding(m metadata) engine.Finding {
	...
	if m.UnderReplicated > 0 {
		f.Status, f.Message = engine.BAD, fmt.Sprintf("%d partizioni under-replicated", m.UnderReplicated)
	} else {
		f.Status, f.Message = engine.OK, "nessuna partizione under-replicated"
	}
	return f
}
...
func (c *Check) groupFinding(ctx context.Context, cl cluster, group string) engine.Finding {
	...
	if err != nil {
		f.Status, f.Message = engine.ERROR, fmt.Sprintf("lag non disponibile: %v", err)
		return f
	}
	switch {
	case c.cfg.LagCrit > 0 && lag >= c.cfg.LagCrit:
		f.Status, f.Message = engine.BAD, fmt.Sprintf("lag totale %d (oltre %d)", lag, c.cfg.LagCrit)
	case c.cfg.LagWarn > 0 && lag >= c.cfg.LagWarn:
		f.Status, f.Message = engine.WARN, fmt.Sprintf("lag totale %d (oltre %d)", lag, c.cfg.LagWarn)
	default:
		f.Status, f.Message = engine.OK, fmt.Sprintf("lag totale %d", lag)
	}
	return f
}

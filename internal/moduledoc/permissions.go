...
var Permissions = map[string]Permission{
	...
	"flow": {NeedsJudgement: true,
		...
		NotNeeded: "No infrastructure permission of its own: the module speaks HTTP to the application, not to the platform underneath it.",
	},
	"pq": {Unauthenticated: true,
		Summary:   "A handful of TLS handshakes per endpoint, closed immediately. No credential.",
		NotNeeded: "No request, no body, no application data: the check reads how the handshake ended, so there is nothing it could change on the far side."},
